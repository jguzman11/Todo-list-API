...
// Use id in request when grab parent element
...
function sendTaskToAPI(item, callback){
  // similar to fetch
  // opens up post request
 var req = new XMLHttpRequest();
//  makes it possible to add stuff
 req.open('POST', '/tasks/add');
//  lets API know its receiving & hanlde json data
 req.setRequestHeader('Content-Type', 'application/json');
 req.send(JSON.stringify({ item: item }));
 
 // event listener (basically listens) for loading and error
 req.addEventListener('load', () => {
  var results = JSON.parse(req.responseText);
  if (results.error) return console.log(results.error);
 ...
 req.addEventListener('error', () => {
   console.log('ohhhhhhh, somthing bad happen!');
   console.log(e);
 });
}
